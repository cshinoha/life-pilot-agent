---
type: note
title: Entry Classification
last_accessed: 2026-02-19
relevance: 0.88
tier: warm
---
# Entry Classification

## Work Domains → Categories

Based on user's work context (see [ABOUT.md](ABOUT.md)):

### Психология / Клиенты
Консультации, сессии, клиенты, группы, тренинги

**Keywords:** клиент, сессия, консультация, группа, тренинг, терапия, гештальт, эриксон

**→ Category:** task (p1-p2) → TaskNotes

### AI & Tech
Инструменты, модели, боты, агенты, автоматизация

**Keywords:** GPT, Claude, бот, агент, API, n8n, vibe coding, автоматизация, MCP

**→ Category:** learning или project → thoughts/

### Контент
Посты, идеи для Telegram и LinkedIn, продвижение практики

**Keywords:** пост, @your_handle, LinkedIn, Facebook, контент, статья, сторис

**→ Category:** idea → thoughts/ideas/ или task если с дедлайном

### Обучение
Испанский, ораторское, курсы, книги

**Keywords:** испанский, курс, книга, учить, практиковать, навык

**→ Category:** learning → thoughts/learnings/

### Personal
Здоровье, отношения, друзья, финансы

**Keywords:** здоровье, зал, зарядка, отношения, друзья, деньги, долг

**→ Category:** task или reflection

### Resources
Полезные ссылки, статьи, инструменты, книги для сохранения

**Keywords:** полезная ссылка, интересная статья, сохранить, добавить в ресурсы, [🔗], [📚], [💎]

**Маркеры:**
- 🔗 = ссылка
- 📚 = материал для чтения
- 💎 = особо ценный ресурс

**→ Category:** resource → MOC/Resources.md

---

## Decision Tree
