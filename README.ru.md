...
sudo systemctl enable --now d-brain-process.timer    # Вечерняя обработка в 21:00
sudo systemctl enable --now d-brain-weekly.timer      # Недельный дайджест
```

### 4. Настройте расписание под себя

Все времена отчётов и коучинг-сессий настраиваются в `src/d_brain/bot/main.py` → `create_scheduler()`. Таймзона по умолчанию — `Europe/Chisinau`. Настройте под свой ритм:

| Событие | Время по умолчанию | Что менять |
|---|---|---|
| Утренний план | 08:00 | cron hour в `send_morning_plan` |
| Вечерняя обработка | 21:00 | `d-brain-process.timer` или job в scheduler |
...
Вдохновлён проектом [smixs/agent-second-brain](https://github.com/smixs/agent-second-brain). Построен с помощью [Claude Code](https://docs.anthropic.com/en/docs/claude-code) и [MCP Protocol](https://modelcontextprotocol.io/).
